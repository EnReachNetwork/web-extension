...
export default function ConAnim(p: { status: StatusConnect; onClick?: MouseEventHandler<SVGSVGElement>; className?: string }) {
    ...
    const animDur = 3;
    const showAnim = p.status == "connecting";
    const conMsg = p.status == "connecting" ? "Connecting..." : p.status == "connected" ? "Connected" : "Connect";
    // return (
    //     <svg
    //         viewBox="0 0 202 202"
    //         fill="none"
    ...
    //         onClick={p.onClick}
    //         className={cn({ "cursor-pointer": p.onClick && (p.status == "idle" || p.status == "error") }, "w-[202px] h-[202px]", p.className)}
    //     >
    //         <circle cx="101" cy="101" r="100.5" stroke={fillColor} stroke-opacity="0.25" stroke-linecap="round" stroke-dasharray="1 8" transform-origin="center center">
    //             {/* {showAnim && <animateTransform attributeName="transform" attributeType="XML" type="rotate" from="0" to="360" dur="12s" repeatCount="indefinite" />} */}
    //         </circle>
    //         <circle cx="101" cy="101" r="92" fill={fillColor} fill-opacity="0.04" transform-origin="center center">
    //             {/* {showAnim && (
    //                 <>
    //                     <animate attributeName="r" from="68" to="100" dur={`${animDur * 2}s`} repeatCount="indefinite" begin={`${animDur}s`} />
    //                     <animate attributeName="fill-opacity" from="0.08" to="0" dur={`${animDur * 2}s`} repeatCount="indefinite" begin={`${animDur}s`} />
    //                 </>
    //             )} */}
    //         </circle>
    //         <circle cx="101" cy="101" r="80" fill={fillColor} fill-opacity="0.08" transform-origin="center center">
    //             {showAnim && (
    //                 <>
    //                     <animate attributeName="r" from="68" to="100" dur={`${animDur * 2}s`} repeatCount="indefinite" />
    //                     <animate attributeName="fill-opacity" from="0.08" to="0" dur={`${animDur * 2}s`} repeatCount="indefinite" />
    //                 </>
    //             )}
    //         </circle>
    //         <circle cx="101" cy="101" r="67.5" fill={fillColor} fill-opacity="0.9" />
    //         <circle cx="101" cy="101" r="48.5" stroke="white" stroke-opacity="0.1">
    //             {showAnim && <animate attributeName="r" from="1" to="68" dur="6s" repeatCount="indefinite" begin="3s" />}
    //         </circle>
    //         <circle cx="101" cy="101" r="20.5" stroke="white" stroke-opacity="0.1">
    //             {showAnim && <animate attributeName="r" from="1" to="68" dur="6s" repeatCount="indefinite" />}
    //         </circle>
    //         {showAnim && (
    //             <use xlinkHref="#round_2983475" x="33" y="33" transform-origin="center center">
    //                 <animateTransform attributeName="transform" attributeType="XML" type="rotate" from="0" to="360" dur={`${animDur}s`} repeatCount="indefinite" />
    //             </use>
    //         )}
    //     </svg>
    // );
    return (
        <svg
            ...
            xmlns="http://www.w3.org/2000/svg"
            onClick={p.onClick}
            className={cn({ "cursor-pointer": p.onClick && (p.status == "idle" || p.status == "error") }, p.className)}
        >
            <circle
                cx="81"
                cy="80.8"
            ...
            <circle cx="80.9999" cy="80.8" r="73.6" fill={fillColor} fill-opacity="0.04" />
            <circle cx="81" cy="80.8" r="64" fill={fillColor} fill-opacity="0.08" />
            ...
            <path
                ...
                fill="white"
            >
                {p.status == "connecting" && <animateTransform attributeName="transform" type="rotate" from="0" to="360" dur="1s" repeatCount="indefinite" />}
                {p.status !== "connecting" && (
                    <>
                        <animateTransform attributeName="transform" type="scale" values="0.3;1;0.3" keyTimes="0;0.5;1" dur="1s" repeatCount="indefinite" />
                        <animate attributeName="fill-opacity" values="0.2;1;0.2" keyTimes="0;0.5;1" dur="1s" repeatCount="indefinite" />
                    </>
                )}
            </path>
        </svg>
